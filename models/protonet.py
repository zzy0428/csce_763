...
from models.attention1 import SelfAttention1
...
class ProtoNet(nn.Module):
    def __init__(self, args,eps=1e-6):
        ...
        self.att1 = SelfAttention1(in_channel=64)
    ...
    def forward(self, support_x, support_y, query_x, query_y):
        ...
        """
        support_x = support_x.view(self.n_way*self.k_shot, self.in_channels, self.n_points) #1 9 2048
        sup_fea1,sup_fea2,sup_fea3 = self.getFeatures(support_x)

        # support_feat = support_feat.view(self.n_way, self.k_shot, -1, self.n_points)
        query_fea1,query_fea2,query_fea3 = self.getFeatures(query_x) #(n_queries, feat_dim, num_points)

        trans1 = self.att1(sup_fea1,query_fea1)
        trans1= trans1.unsqueeze(0)
        trans2 = self.att1(sup_fea2,query_fea2)
        trans2 = trans2.unsqueeze(0)
        trans3 = self.att1(sup_fea3,query_fea3)
        trans3 = trans3.unsqueeze(0)
        support_feat = torch.cat([trans1, trans2, trans3], dim=2)

       # print(support_feat.shape)
        # mix1 = self.MixStyle(sup_fea1,query_fea1)
        # mix2 = self.MixStyle(sup_fea2, query_fea2)
        # mix3 = self.MixStyle(sup_fea3, query_fea3)
        # print(mix3.shape) 1 1 64 2048
       # support_feat = torch.cat([mix1, mix2, mix3], dim =2)  #torch.Size([1, 1, 192, 2048])


        query_feat = torch.cat([query_fea1, query_fea2, query_fea3], dim=1)
    ...
    def getFeatures(self, x):
        ...
        if self.use_attention:
            ...
            return feat_level1, att_feat, feat_level3
        else:
            # return self.base_learner(self.encoder(x))
            feat_level1, feat_level2 = self.encoder(x)
            feat_level3 = self.base_learner(feat_level2)
            map_feat = self.linear_mapper(feat_level2)
            return feat_level1, map_feat, feat_level3
    ...
    def getMaskedFeatures(self, feat, mask):
        ...
        # print(feat.shape) torch.Size([1, 1, 192, 2048])

        # mask = mask.unsqueeze(2) # 2048
        masked_feat = torch.sum(feat * mask, dim=3) / (mask.sum(dim=0) + 1e-5)
        return masked_feat
