...
from rgbd2depth import generate_pointcloud
...
class MyDataset(Dataset):
    def __init__(self, data_path, dataset_name, cvfold=0, num_episode=50000, n_way=3, k_shot=5, n_queries=1,
        ...
        self.data_path = './datasets/S3DIS/scenes/blocks_bs1_s1'
        self.n_way = n_way
        self.k_shot = k_shot
    ...
    def __getitem__(self, index, n_way_classes=None):
        ...
        _, _, query_ptclouds, query_labels = self.generate_one_episode(sampled_classes)
        index_num = sampled_classes[0]
        support_ptclouds, support_masks = generate_pointcloud(index_num)
...
class MyTestDataset(Dataset):
    def __init__(self, data_path, dataset_name, cvfold=0, num_episode_per_comb=100, n_way=1, k_shot=5, n_queries=1,
                       num_point=4096, pc_attribs='xyz', mode='test'):
...
def batch_test_task_collate(batch):
    ...
    # print(batch_query_ptclouds.shape)

    data = [torch.from_numpy(batch_support_ptclouds).transpose(1,0), torch.from_numpy(batch_support_masks),
